<template>
  <div class="calendar">
    ...
    <div class="calendar__body">
      <CalendarCell v-for="(day, i) of days" :key="'day_' + i" v-bind="day" @select="click">
        <CalendarBadge v-for="(badge, i) of getBadges(day)" :key="`badge_${i}`" v-bind="badge" />
      </CalendarCell>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    click(day) {
      ...
      var dd = this.select._d.getDate();
      var mm = this.select._d.getMonth() + 1;
      var yyyy = this.select._d.getFullYear();
      if (dd < 10) {
        dd = "0" + dd;
      }
      if (mm < 10) {
